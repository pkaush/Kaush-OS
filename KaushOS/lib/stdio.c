...
static const char *parse_conversion (const char *format,
                                     struct printf_conversion *,
                                     va_list *);
static void format_integer (uintmax_t value, BOOLEAN is_signed, BOOLEAN negative, 
        ...
        case 'p':
          {
            ...
            c.flags = POUND;
            format_integer ((ULONG_PTR) p, FALSE, FALSE,
                            &base_x, &c, output, aux);
          }
          break;
...
/* Performs an integer conversion, writing output to OUTPUT with
   ...
   are in C. */
static void
format_integer (uintmax_t value, BOOLEAN is_signed, BOOLEAN negative, 
                const struct integer_base *b,
...
/* Dumps the SIZE bytes in BUF to the console as hex bytes
   ...
   alongside. */   
void
hex_dump (ULONG_PTR ofs, const void *buf_, size_t size, BOOLEAN ascii)
{
