...
static void format_integer(uintmax_t value, BOOLEAN is_signed, BOOLEAN negative, const struct integer_base *,
...
void __vprintf(const char *format, va_list args, void (*output)(char, void *), void *aux)
{
    for (; *format != '\0'; format++)
    {
        ...
        case 'p': {
            ...
            c.flags = POUND;
            format_integer((ULONG_PTR)p, FALSE, FALSE, &base_x, &c, output, aux);
        }
        break;
...
/* Performs an integer conversion, writing output to OUTPUT with
   ...
   are in C. */
static void format_integer(uintmax_t value, BOOLEAN is_signed, BOOLEAN negative, const struct integer_base *b,
