...
VOID PsSetCurrentThread(PETHREAD Thread)
{
    KiSetCurrentThread((PKTHREAD)Thread);
}

VOID PsSetTheadPriority(PETHREAD Thread, PROCESS_PRIORITY Priority)
{

    Thread->tcb.Priority = Priority;
}

static BOOL PsIsLastThread(IN PETHREAD Thread)
{
    PEPROCESS Process;
...
VOID PsExitThread(IN PETHREAD Thread)
{
...
KSTATUS
PspAllocateThread(IN PEPROCESS Process, OUT PETHREAD *Thread, BOOL IsClone)
...
KSTATUS
PspInsertThread(IN PEPROCESS Process, IN PETHREAD Thread)
