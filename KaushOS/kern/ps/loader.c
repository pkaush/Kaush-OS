...
static BOOL ELFValidateSegment(const struct Proghdr *phdr, size_t FileSize)
{
    ...
    for (hnum = 0; hnum < elf_head.e_phnum; hnum++)
    {
        ...
        case PT_LOAD:
            ...
            if (ELFValidateSegment(&phdr, fp->fsize))
            {
                ...
                MmMapSegment(Process->pcb.DirectoryTableBase, phdr.p_va, phdr.p_memsz, PTE_W | PTE_U);
