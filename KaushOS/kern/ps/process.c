...
typedef LIST_HEAD(_EPROCESS_LIST, _EPROCESS) EPROCESS_LIST, *PEPROCESS_LIST;
...
VOID PsIncProcessRundownProtect(IN PEPROCESS Process)
{
...
VOID PsDecProcessRundownProtect(PEPROCESS Process)
{
    LONG Ret = 0;
...
static INLINE HANDLE PsGetCurrentProcessId()
{
    PEPROCESS Process;
...
VOID PspAttachProcess(IN PEPROCESS NewProcess, OUT PRKAPC_STATE ApcState)
/*
    Description:
...
VOID PspDetachProcess(IN PRKAPC_STATE ApcState)
/*
    Description:
...
///////////////////////////////
// Insert and Remove
static INLINE KSTATUS PspRemoveProcess(IN PEPROCESS Process)
{
    KIRQL OldIrql;
...
static INLINE KSTATUS PspInsertProcess(IN PEPROCESS Process)
{
    KIRQL OldIrql;
...
/////////////////
// Allocate and Free
static INLINE KSTATUS PspFreeProcess(IN PEPROCESS Process)
{
    ASSERT(Process);
...
static INLINE KSTATUS PspReadBinary(PCHAR FileName, PVOID *Buffer, PLONG BinarySize)
{
...
static INLINE KSTATUS PspAllocateProcess(IN PEPROCESS Parent, IN PCHAR Binary, IN size_t size,
                                         IN POBJECT_ATTRIBUTES ObjectAttributes, IN KPROCESSOR_MODE ProbeMode,
                                         OUT PEPROCESS *NewProcess)
{
    ...
    Process = NULL;
    Status = ObCreateObject(ProbeMode, PsProcessType, ObjectAttributes, sizeof(EPROCESS), &Process);
    ...
    if (IsCloneProcess)
    {
        ...
        Status = MmCloneProcessAddressSpace(Parent->pcb.DirectoryTableBase, PageDirectory);
...
KSTATUS
PsCreateProcess(IN KPROCESSOR_MODE ProbeMode, IN POBJECT_ATTRIBUTES ObjectAttributes OPTIONAL, IN PCHAR Binary,
                IN size_t size, IN PHANDLE ChildProcessid, IN PEPROCESS *CreatedProcess)
...

{
    ...
    Status = PspAllocateProcess(PsGetCurrentProcess(), Binary, size, ObjectAttributes, ProbeMode, &NewProcess);
    ...
    ASSERT1(ProcessGetCleanupFlags(NewProcess) == PROCESS_FLAG_ALLOCATED, ProcessGetCleanupFlags(NewProcess));
    ...
    Status = PspCreateThread(NewProcess, "Test ROutine", NewProcess->pcb.ProcessEntryPoint, NULL, NULL);
    ...
    Log(LOG_DEBUG, "[%x] Process Created by Parent [%x]\n", NewProcess->UniqueProcessId, NewProcess->ParentProcessId);
...
static INLINE KSTATUS PspExitProcess(IN PEPROCESS Process)
...
KSTATUS
KExitProcess(IN HANDLE ProcessId, IN ULONG ExitStatus)
{
    HANDLE Pid;
...
static INLINE KSTATUS PspCloseProcess(IN PEPROCESS Process)
{
    KSTATUS Status;
    PETHREAD Thread;
...
VOID PspIdleThread(IN PVOID StartContext)
...
VOID PspTestThread(PVOID StartContext)
{
    ...
    KeWaitForSingleObject(&TestWaitLock, Executive, KernelMode, FALSE, NULL);
...
#ifdef TEST_MUTANT
    KeReleaseMutant(&TestWaitLock, 0, FALSE, FALSE);

#else
    KeReleaseSemaphore(&TestWaitLock, 0, 1, FALSE);
#endif
...
static VOID ProcessThreadLocksTests();
...
static VOID ProcessThreadLocksTests()
{
    ...
    */
    ...
    for (count = 0; count < 10; count++)
    {
        ...
        PspCreateThread(PsInitialSystemProcess, "TestThread", &PspTestThread, (PVOID)count, &Thread);
    }
...
VOID PsSystemProcessStart(IN PVOID StartContext)

{
    ...
    */
    ...
    for (count = 0; count < NumProcs; count++)
    {
        ...
        PspCreateThread(PsInitialSystemProcess, "IdleThread", PspIdleThread, NULL, &Thread);
...
static VOID INLINE SyspCreateFirstSystemThread(IN PEPROCESS Process, OUT PETHREAD *Thread)
{
    ...
    NewThread = MmAllocatePoolWithTag(NonPagedPool, sizeof(ETHREAD), 'RHTE');
...
VOID PsCreateSystemProcess()
{
    ...
    InitializeObjectAttributes(&ObjectAttributes, NULL, OBJ_PERMANENT | OBJ_KERNEL_HANDLE, NULL, NULL);

    Status = PspAllocateProcess(NULL, NULL, 0, &ObjectAttributes, KernelMode, &NewProcess);
    ...
    LIST_INSERT_TAIL(&NewProcess->ThreadListHead, Thread, Process_link);

#if 1
    Status = KiCreateThreadContext(&Thread->tcb, NULL, PsSystemProcessStart, Thread);
#else

    Status = SyspCreateThreadContext(&Thread->tcb, PsSystemProcessStart, "Starting  SystemProcess");
    ...
    */
    ...
    KiContextSwitch(Thread->tcb.StackPointer, &StackPointer);
