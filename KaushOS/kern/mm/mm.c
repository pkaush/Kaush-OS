...
PHYSICAL_ADDRESS INLINE MmGetGlobalCr3()
{ // Tmp function for use in Attach Process
    return (GlobalCr3);
}

PVOID INLINE MmGetGlobalPgdir()
{
    return (GlobalPgdir);
}

static VOID MmInitializeSystemPtes();
...
static KSTATUS MmAllocatePFN(PMMPFN *pfn, PMMPTE PteAddress)
{
    PMMPFN Page;
...
*/
VOID static MiDecReferencePFN(PMMPFN pfn)
{
    if (--(pfn->RefCount) == 0)
        MiFreePFN(pfn);
}
...
static INLINE ULONG MiPFNtoFrameNumber(PMMPFN pfn)
{
    return (pfn - PfnDataBase);
}

static INLINE PHYSICAL_ADDRESS MiPFNtoPhysicalAddress(PMMPFN pfn)
{
    PHYSICAL_ADDRESS FrameNumber = MiPFNtoFrameNumber(pfn);
    return (FrameNumber << PGSHIFT);
}

static INLINE PMMPFN MiPhysicalAddresstoPFN(PHYSICAL_ADDRESS PAddress)
{
    return (&PfnDataBase[PAddress >> PTXSHIFT]);
}

static INLINE PMMPFN MmGetPfnFromPte(PMMPTE Pte)
{
...
*/
static PVOID MiInitBootAlloc(IN ULONG NumBytes, IN ULONG align)
{
    extern char end[];
    PVOID VirtAddress;
...
*/
static INLINE void MiBootMapMemory(IN OUT PMMPTE PgDir, IN ULONG_PTR Address, IN size_t size,
                                   IN PHYSICAL_ADDRESS PhyAddress, ULONG Perm)
{
    PMMPTE page_table;
    int i, j;
...
// static INLINE
BOOL IsCurrentPageDirectory(PVOID PageDirectory)
{
...
VOID InvalidateTLB(PMMPTE PageDirectory, PVOID Address)
{
    /*
...
KSTATUS
MiInsertPage(IN PMMPTE PageDirectory, IN PVOID Address, IN PMMPFN pPfn, IN ULONG Perm)
{
    PMMPTE Pte;
    PMMPTE Pde;
    KSTATUS status;
    ...
    */
    if (!MmIsPageEntryValid(Pde))
    {
        ...
        {
            PageDirectory[PDX(Address)].u.Long = MiPFNtoPhysicalAddress(pPfn1) | PTE_W | PTE_P;
        }
...
KSTATUS
MmMapSegment(IN PVOID PageDirectory, IN PVOID Address, IN size_t Length, IN ULONG Perm)
{
...
VOID MmVirtualFree(IN PVOID VirtualAddress, IN size_t size)
{
    size_t i;
...
KSTATUS
MmVirtualAlloc(IN PVOID VirtualAddress, IN size_t size, IN ULONG perm)
{
    size_t i;
    PMMPFN Page;
    KSTATUS status = STATUS_SUCCESS;
...
VOID MmFreeSystemPtes(IN PVOID Address, IN ULONG NumberOfPages)
{
    size_t start;
    KIRQL OldIrql;
...
VOID MmAttachToAddressSpace(PVOID PageDirectory)
{
...
VOID MmDeleteProcessAddressSpace(PVOID PageDirectory)
/*++
...
KSTATUS
MmCloneProcessAddressSpace(IN PMMPTE ClonePageDirectory, IN PMMPTE NewPageDirectory)
/*++
