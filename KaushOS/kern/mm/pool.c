...
void Test_Pool(POOL_TYPE pooltype);
...
VOID MmInitializeAllPools()
{
    ...
    MmInitializePool((PVOID)MMPAGEPOOL_START, MMPAGEPOOL_SIZE, PagedPool);

    // Initialize Non Paged Pool
    MmInitializePool((PVOID)MMNONPAGEPOOL_START, MMNONPAGEPOOL_SIZE, NonPagedPool);
#ifdef DEBUG
    Test_Pool(PagedPool);
