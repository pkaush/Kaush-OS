...
typedef KSTATUS IO_COMPLETION(IN PDEVICE_OBJECT DeviceObject, IN PIRP pIrp, IN PVOID Context);

typedef IO_COMPLETION *PIO_COMPLETION;

typedef BOOLEAN DRIVER_CANCEL(IN PIRP Irp);
...
PIRP IoCreateIrp(IN PFILE_OBJECT pFileObj, IN UCHAR MajorFunction, IN UCHAR MinorFunction, IN PVOID ReadBuffer,
                 IN size_t ReadBufferSize, IN PVOID WriteBuffer, IN size_t WriteBufferSize, IN ULONG Offset);

VOID IoFreeIRP(PIRP pIrp);
...
typedef VOID DRIVER_UNLOAD(IN PDRIVER_OBJECT DriverObject);
typedef DRIVER_UNLOAD *PDRIVER_UNLOAD;

typedef VOID DRIVER_STARTIO(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp);

typedef DRIVER_STARTIO *PDRIVER_STARTIO;

typedef KSTATUS DRIVER_INITIALIZE(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath);
typedef DRIVER_INITIALIZE *PDRIVER_INITIALIZE;

typedef KSTATUS DRIVER_DISPATCH(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp);
typedef DRIVER_DISPATCH *PDRIVER_DISPATCH;

typedef KSTATUS DRIVER_AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject);
...
PIRP IoBuildIrp(IN PFILE_OBJECT pFileObj, IN UCHAR MajorFunction, IN UCHAR MinorFunction, IN PVOID Buffer,
                IN size_t BufferSize, IN ULONG Offset);
...
KSTATUS
KQueryFileStats(IN HANDLE Handle, IN PVOID Buffer, IN ULONG NumberOfBytes);
