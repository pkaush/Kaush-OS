...
PKPRCB
KiGetPrcb(IN ULONG ProcessorNumber)
{
    PKPCR Pcr;
...
VOID KiLockProcessorRaisetoHighLevel(IN ULONG ProcessorNumber, OUT PKIRQL OldIrql)
{
...
VOID KiUnLockProcessorFromHighLevel(IN ULONG ProcessorNumber, IN KIRQL OldIrql)
{
...
VOID INLINE KiIncrementCurrentTimerTicks()
{
    PKPRCB Prcb = KiGetCurrentPrcb();
    Prcb->TimerTicks++;
}
...
VOID KiSetIdleThread(IN PKTHREAD Thread, IN LONG ProcessorNumber)
{
...
VOID KiRequestSoftwareInterrupt(KIRQL Irql)
{
    PKPRCB Prcb;
    ASSERT1((Irql == DISPATCH_LEVEL) || (Irql == APC_LEVEL), Irql);
...
VOID KiIncrementCurrentProcessorsIdleTime()
{
...
VOID KiIncrementCurrentProcessorsDpcTime()
{
...
VOID KiIncrementCurrentProcessorsIsrTime()
{
...

{
    ...
    KiLockProcessorRaisetoHighLevel(KeGetCurrentProcessorNumber(), &OldIrql);
    /*
        Copy our MODEL GDT
    */
    ...
    */
    ...
    Pcr->GDT[GD_TSS >> 3] = SEG16(STS_T32A, (ULONG_PTR)(&Pcr->TSS), sizeof(KTASKSTATE), 0);
...
VOID KiUpdateCurrentTSS(PVOID Stack)
{
    PKPCR Pcr;
...
VOID KiInitializeAllProcessors()
{
}
...
VOID KiInitProcessorPhase0()
{
    ...
    {
        memset(&KiProcessorBlock[count], 0, sizeof(KPCR));
