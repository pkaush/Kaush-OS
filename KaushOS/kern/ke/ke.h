...
ULONG
KeQuerySpinLock(IN PKSPIN_LOCK SpinLock, OUT PKTHREAD *Thread);
...
VOID KiSetCurrentIrql(IN KIRQL NewIrql);
...
typedef VOID (*PKDEFERRED_ROUTINE)(IN PKDPC Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1,
...
VOID KiLockDispatcherDataBase(OUT PKIRQL OldIrql);

VOID KiUnLockDispatcherDataBase(IN KIRQL OldIrql);

VOID KiLockDispatcherDataBaseAtDpcLevel();

VOID KiUnLockDispatcherDataBaseFromDpcLevel();

ULONG
KiQueryDispatcherDatabase(OUT PKTHREAD *Thread);
...
typedef LIST_HEAD(_KWAIT_BLOCK_LIST, _KWAIT_BLOCK) KWAIT_BLOCK_LIST, *PKWAIT_BLOCK_LIST;
...
VOID KeInitializeMutant(IN OUT PKMUTANT Mutant, IN BOOL InitialOwner);

VOID KeInitializeSemaphore(IN OUT PRKSEMAPHORE Semaphore, IN LONG Count, IN LONG Limit);
...
typedef LIST_HEAD(_KTHREAD_LIST, _KTHREAD) KTHREAD_LIST, *PKTHREAD_LIST;

typedef LIST_HEAD(_KPROCESS_LIST, _KPROCESS) KPROCESS_LIST, *PKPROCESS_LIST;
...
VOID KiSetCurrentThread(IN PKTHREAD Thread);

VOID KiCreateCurrentGDT();

KSTATUS
KiCreateThreadContext(IN PKTHREAD Tcb, IN PKTHREAD CloneTcb, IN PKSTART_ROUTINE StartRoutine, IN PVOID StartContext);

ULONG
KiContextSwitch(IN ULONG_PTR NextStackPointer, IN ULONG_PTR *CurrentStackPointer);

VOID KiAttachProcess(IN PKPROCESS NewProcess);
