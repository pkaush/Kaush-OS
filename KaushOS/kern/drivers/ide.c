...
void disk_read_internal(struct disk *d, disk_sector_t sec_no, void *buffer)
{
    struct channel *c;
...
DRESULT
disk_read(BYTE Drive,         /* Physical drive number */
...
/* Write sector SEC_NO to disk D from BUFFER, which must contain
   ...
   Internally synchronizes accesses to disks, so external
   per-disk locking is unneeded. */
void disk_write_internal(struct disk *d, disk_sector_t sec_no, const void *buffer)
{
    struct channel *c;
...
DRESULT
disk_write(BYTE Drive,         /* Physical drive number */
