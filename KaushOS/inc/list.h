...
#define LIST_INIT_ENTRY(elm, link)                                                                                     \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define LIST_NEXT(head, elm, link) ((LIST_NEXT_INTERNAL(elm, link) == LIST_FIRST(head)) ? NULL : ((elm)->link.le_next))

#define LIST_PREV(head, elm, link)                                                                                     \
    ((LIST_PREV_INTERNAL(elm, link) == LIST_LAST(head, link)) ? NULL : ((elm)->link.le_prev))

#define LIST_FOREACH(var, head, link) for ((var) = LIST_FIRST((head)); (var); (var) = LIST_NEXT(head, (var), link))

// insert elm after the listelm in the list

#define LIST_INSERT_AFTER(head, listelm, elm, link)                                                                    \
    do                                                                                                                 \
    {                                                                                                                  \
...
#define LIST_INSERT_HEAD(head, elm, link)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        if (LIST_EMPTY(head))                                                                                          \
        {                                                                                                              \
...
#define LIST_INSERT_TAIL(head, elm, link)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        if (LIST_EMPTY(head))                                                                                          \
        {                                                                                                              \
...
#define LIST_REMOVE(head, elm, link)                                                                                   \
    do                                                                                                                 \
    {                                                                                                                  \
        ...
        LIST_NEXT_INTERNAL(LIST_PREV_INTERNAL(elm, link), link) = LIST_NEXT_INTERNAL(elm, link);                       \
        LIST_PREV_INTERNAL(LIST_NEXT_INTERNAL(elm, link), link) = LIST_PREV_INTERNAL(elm, link);                       \
        LIST_NUM_ELEMENTS(head)--;                                                                                     \
    } while (0)

#define LIST_REMOVE_HEAD(head, elm, link)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        if (LIST_EMPTY(head))                                                                                          \
        {                                                                                                              \
...
#define LIST_REMOVE_TAIL(head, elm, link)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        if (LIST_EMPTY(head))                                                                                          \
        {                                                                                                              \
...
#define InitializeSListHead(_SListHead) (_SListHead)->Next = 0
//			(_SListHead)->Alignment = 0

static INLINE PSLIST_ENTRY InterlockedPushEntrySList(IN PSLIST_HEADER ListHead, IN PSLIST_ENTRY ListEntry)
{
    ...
    do
    {
        ...
        FirstEntry = (PVOID)InterlockedCompareExchangePointers((PLONG)Head, (LONG)Entry, (LONG)FirstEntry);
    } while (FirstEntry != NextEntry);

    return FirstEntry;
}

static INLINE PSLIST_ENTRY InterlockedPopEntrySList(IN PSLIST_HEADER ListHead)
{
    ...
    do
    {
        ...
        NextEntry = FirstEntry;
        FirstEntry = (PVOID)InterlockedCompareExchangePointers((PLONG)Head, (LONG)FirstEntry->Next, (LONG)FirstEntry);
...
static INLINE PSINGLE_LIST_ENTRY ExInterlockedFlushSList(IN PSLIST_HEADER ListHead)
{
    return (PVOID)InterlockedExchange((PLONG)&ListHead->Next.Next, (LONG)NULL);
}

static INLINE USHORT QueryDepthSList(IN PSLIST_HEADER ListHead)
{
...
static INLINE VOID InsertHeadList(IN PLIST_ENTRY ListHead, IN PLIST_ENTRY Entry)
{
